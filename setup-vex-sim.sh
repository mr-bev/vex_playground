...
#       even outside this project directory.  This is needed to work
...
#       The same pyproject.toml also sets  native-tls = true  under
#       [tool.uv], which makes every uv command run inside this project
#       directory use the system TLS store — no shell config required.
...
#
#  NOTE ON native-tls (and why we set it three different ways)
...
#    school's, causing  "invalid peer certificate: unknownIssuer"  errors.
#    Telling uv to use native TLS makes it consult the operating system's
#    certificate store, which *does* trust the school's CA.
#
#    There are three ways to turn native TLS on:
#      1. native-tls = true  under [tool.uv]  in pyproject.toml.
#         This is per-project and applies to every uv command run from
#         this directory — including `uv run` in a fresh terminal that
#         never sourced any shell profile.  This is the PRIMARY fix.
#      2. UV_NATIVE_TLS=1  in the shell environment.
#         We export this in the student's shell profile as a backup for
#         uv commands run OUTSIDE this project directory.
#      3. --native-tls  passed explicitly on each uv invocation.
#         Used inside this script for the steps that run BEFORE the
#         pyproject.toml exists (e.g. `uv python install`).
#
#    We do all three because each closes a different gap.
#
#  REQUIREMENTS
...
if [ -f "$PYPROJECT_FILE" ]; then
    ...
    # Make sure native-tls = true is present under [tool.uv].  Older
    # versions of this script didn't set it, and students whose pyproject
    # was generated by one of those would hit  "invalid peer certificate:
    # unknownIssuer"  on the school network when they later ran  uv run
    # in a terminal that hadn't picked up UV_NATIVE_TLS=1 from .zshrc.
    if ! grep -qE '^\s*native-tls\s*=\s*true' "$PYPROJECT_FILE" 2>/dev/null; then
        info "  Patching ${PYPROJECT_FILE} to add native-tls = true under [tool.uv]"
        awk '
            BEGIN { inserted = 0 }
            /^\[tool\.uv\]/ && !inserted {
                print
                print "# Use the OS TLS store (school firewall installs its own CA)."
                print "native-tls = true"
                inserted = 1
                next
            }
            { print }
        ...
        success "  Patched ${PYPROJECT_FILE}"
    fi
else
...
package = false
# Use the operating system's TLS certificate store rather than uv's
# bundled CA bundle.  The school firewall MITMs HTTPS with a custom CA
# that's installed in the system keychain but NOT in uv's bundle, so
# without this setting students hit  "invalid peer certificate:
# unknownIssuer"  whenever they run  uv run … .  Setting it here (rather
# than relying on the UV_NATIVE_TLS env var) means every uv command in
# this directory works regardless of which terminal or shell they use.
native-tls = true
PYPROJECT_EOF
